...
class MailBox extends EventEmitter
{
	constructor(server, opts)
	{
		...
		this.closed = false;
		this.opts = opts;
		CONNECT_TIMEOUT = opts.connectTime || 2000;
	...
	connect(tracer, cb)
	{
		...
		this.socket.on('publish', pkg =>
		{
			...
			{
				MailboxUtility.UpgradeHandshake(this, pkg.payload);
				return cb();
			}
			try
			{
...
class MailboxUtility
{
	...
	static UpgradeHandshake(mailbox, msg)
	{
		const servicesMap = JSON.parse(msg.toString());
