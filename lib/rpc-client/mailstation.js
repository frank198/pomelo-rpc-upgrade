...
class MailStation extends EventEmitter
{
	...
	addServer(serverInfo)
	{
		...
		if (!_.includes(this.serversMap[type], id))
		{
	...
	dispatch(...args)
	{
		...
		tracer && tracer.info('client', fileName, 'dispatch', 'dispatch rpc message to the mailbox');
		tracer && (tracer.cb = cb);
		if (this.state !== STATE_STARTED)
		{
			tracer && tracer.error('client', fileName, 'dispatch', 'client is not running now');
		...
		if (!mailbox)
		{
			tracer && tracer.debug('client', fileName, 'dispatch', 'mailbox is not exist');
			...
			{
				tracer && tracer.error('client', fileName, 'dispatch', `fail to find remote server:${serverId}`);
		...
		{
			tracer && tracer.debug('client', fileName, 'dispatch', 'request add to connecting');
		...
		{
			tracer && tracer.info('client', fileName, 'send', 'get corresponding mailbox and try to send message');
			const mailbox = this.mailboxes[serverId];
			if (err)
			{
			...
			if (!mailbox)
			{
				tracer && tracer.error('client', fileName, 'send', `can not find mailbox with id:${serverId}`);
	...
	connect(tracer, serverId, cb)
	{
		...
		mailbox.connect(tracer, (err) =>
		{
			if (err)
			{
				tracer && tracer.error('client', fileName, 'connect', `fail to connect to remote server: ${serverId}`);
...
class MailstationUtility
{
	...
	static DoFilter(tracer, err, serverId, msg, opts, filters, index, operate, cb)
	{
		...
		{
			tracer && tracer.info('client', fileName, 'DoFilter', `do ${operate} filter ${filters[index].name}`);
		}
	...
	{
		tracer && tracer.info('client', fileName, 'LazyConnect', 'create mailbox and try to connect to remote server');
	...
	{
		tracer && tracer.info('client', fileName, 'AddToPending', 'add pending requests to pending queue');
		...
		{
			tracer && tracer.debug('client', fileName, 'AddToPending', `station pending too much for: ${serverId}`);
	...
	{
		tracer && tracer.info('client', fileName, 'FlushPending', 'flush pending requests to dispatch method');
		...
		if (!mailbox)
		{
			tracer && tracer.error('client', fileName, 'FlushPending', `fail to flush pending messages for empty mailbox: ${serverId}`);
