...
class MailStation extends EventEmitter
{
	...
	dispatch(tracer, serverId, msg, opts, cb)
    {
		...
		if (!mailbox)
        {
			...
			MailstationUtility.AddToPending(tracer, this, serverId, Array.from(arguments));
			return;
		}
		...
		const send = (tracer, err, serverId, msg, opts) =>
        {
			...
			mailbox.send(tracer, msg, opts, () =>
            {
				...
				MailstationUtility.DoFilter(tracer, null, serverId, msg, opts, this.afters, 0, 'after', (tracer, err, serverId, msg, opts) =>
                {
					if (err)
                    {
	...
	connect(tracer, serverId, cb)
    {
		...
		mailbox.connect(tracer, (err) =>
        {
			if (err)
            {
				tracer.error('client', __filename, 'connect', `fail to connect to remote server: ${serverId}`);
